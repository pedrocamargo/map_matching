...
class MapMatcher():
    ...
    def find_stops(self):
        ...
        if self.trip.error is None and self.trip.stops_algorithm in ['Maximum space', 'Delivery stop']:
            ...
            if self.trip.stops_algorithm == 'Maximum space':
                ...
                for i in xrange(0, self.trip.gps_trace.index.shape[0] - 1):
                    ...
                    if ct > par_time and cd > min_distance or cd > max_distance:
                        cd = 0
                        ct = 0
                        self.trip.stops.append([self.trip.gps_trace['latitude'].iloc[i], self.trip.gps_trace['longitude'].iloc[i],
                                           self.trip.gps_trace['timestamp'].iloc[i], 0, 0.0])

                if self.trip.stops[-1][0:3] != [self.trip.gps_trace['latitude'].iloc[-1], self.trip.gps_trace['longitude'].iloc[-1],
            ...
            self.trip.stops = pd.DataFrame(self.trip.stops,
                                      columns=['latitude', 'longitude', 'stop_time', 'duration', 'coverage'])
    ...
    def find_route(self):
        ...
        if all_links:
            ...
            for i, q in enumerate(all_nodes):
                ...
                df.append([all_arrives_tstamps[i], all_leaves_tstamps[i], self.network.nodes.index[q], link, direc,
                           self.network.nodes.Y[q], self.network.nodes.X[q], mp])
                if all_links[i] > 0:
